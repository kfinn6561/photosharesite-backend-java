...
public class PhotoShareSiteApplication extends Application<PhotoShareSiteConfiguration> {
  ...
  public void run(final PhotoShareSiteConfiguration configuration, final Environment environment) {
    ...
    final LookupUserResource lookupUserResource =
        new LookupUserResource(new InsertOrSelectUserAccess(jdbi));
    environment.jersey().register(lookupUserResource);

    // create and register GetFilesResource
    final GetFilesResource getFilesResource = new GetFilesResource(new SelectFilesAccess(jdbi));
    environment.jersey().register(getFilesResource);

    // create and register UploadFilesResource
    final UploadFilesResource uploadFilesResource =
        new UploadFilesResource(
            s3Client,
    ...
    final DeleteFileResource deleteFileResource =
        new DeleteFileResource(
